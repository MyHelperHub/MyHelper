import { ipcInstallPlugin, ipcUninstallPlugin } from "@/api/ipc/plugin.api";
import {
  ipcGetPluginConfigValue,
...
export const getPluginConfig = async (
  ...
  try {
    ...
    // 如果指定了具体的插件ID
    if (keys.length > 1) {
      const result = (await ipcGetPluginConfigValue(
        keys[1],
      )) as PluginConfigData[];
 ...
 */
export const deletePluginConfig = async (
  keys: Array<string>,
): Promise<void> => {
  try {
